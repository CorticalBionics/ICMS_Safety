...
tld = "P:\users\tgh28\Experiments\Longitudinal_ICMS\archive\vm_data";

% List of sessions/collections
vm_collection_list = dir(fullfile(tld, '*.json'));
for collection = 1:size(vm_collection_list,1)
    if contains(vm_collection_list(collection).name, 'session') == 0
        continue
    end
    [~,b,c] = fileparts(vm_collection_list(collection).name);
    fname = sprintf('%s.mat', b);
