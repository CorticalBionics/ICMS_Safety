...
%% Correlations
fnames = {'vmin', 'vinter', 'vmax'};

[r, rp] = deal(NaN(64, num_part, 3));
for p = 1:num_part
    idx = cellfun(@(c) ~isempty(c), {cleaning_data{p}.vmin});
    x = [cleaning_data{p}(idx).date];
    for f = 1:3
        ...
        [r(:,p,f), rp(:,p,f)] = corr(datenum(x'), ty', 'Rows', 'pairwise', 'type', 'spearman'); %#ok<DATNM>
    end
end
...
%% Compare waveforms on two sequential days
p = 3;
i = 30;

clf;
subplot(1,3,1);
    plot(cleaning_data{p}(i).wf'); ylim([-2 2])
    title(string(cleaning_data{p}(i).date))

subplot(1,3,2);
    plot(cleaning_data{p}(i+1).wf'); ylim([-2 2])
    title(string(cleaning_data{p}(i + 1).date))

subplot(1,3,3); hold on
    x1 = median(cat(2, cleaning_data{p}(i-5:i).vmin), 2, 'omitnan');
    x2 = median(cat(2, cleaning_data{p}(i+1:i+6).vmin), 2, 'omitnan');
    scatter(x1, x2)
...
%% Correlations in waveform values over time
p = 3;
idx = cellfun(@isempty, {cleaning_data{p}.vmin});

clf;
subplot(1,3,1);
    x = cat(2, cleaning_data{p}(~idx).vmin);
    r = corr(x, 'Rows', 'pairwise', 'Type', 'Spearman');
    imagesc(r)
    clim([-1 1])
    title(sprintf("Min: r = %0.2f", mean(r, 'all', 'omitnan')))

subplot(1,3,2);
    x = cat(2, cleaning_data{p}(~idx).vinter);
    r = corr(x, 'Rows', 'pairwise', 'Type', 'Spearman');
    imagesc(r)
    clim([-1 1])
    title(sprintf("Inter: r = %0.2f", mean(r, 'all', 'omitnan')))

subplot(1,3,3);
    x = cat(2, cleaning_data{p}(~idx).vmax);
    r = corr(x, 'Rows', 'pairwise', 'Type', 'Spearman');
    imagesc(r)
    clim([-1 1])
    colorbar
    title(sprintf("Max: r = %0.2f", mean(r, 'all', 'omitnan')))
