    ...
    if contains(data_sorted(i).Subject, participant) %Check if correct participant
        ...
        for c = 1:length(chs)
            ...
            if data_sorted(i).Amp == 10
                ...
                vmin_10{ch} = [vinter_10{ch}; data_sorted(i).vmin(ch)];
            ...
            elseif data_sorted(i).Amp == 20
                ...
                vmin_20{ch} = [vinter_20{ch}; data_sorted(i).vmin(ch)];
...
for i = 1:2
    ...
    y_min = y_min(~nan_idx);
    y_min = movmedian(y_inter, 10);

    y_max = y_max(~nan_idx);
    y_max = movmedian(y_inter, 10);
    ...
    [ch_corrs_vmin(ch), cp_vmin] = corr(x, y_min, 'Rows', 'complete');
    p_vmin = polyfit(x, y_min, 1);
    ch_coeff_vmin(ch) = p_vmin(1);
    plot([min(x), max(x)], polyval(p_vmin, [min(x), max(x)]), 'Color', c_vmin, 'LineStyle', line_style);  % Fit line in purple
    ...
    plot([min(x), max(x)], polyval(p_vinter, [min(x), max(x)]), 'Color', c_vinter, 'LineStyle', line_style);  % Fit line in orange
    ...
    plot([min(x), max(x)], polyval(p_vmax, [min(x), max(x)]), 'Color', c_vmax);  % Fit line in green
...
%         y_inter = movmedian(y_inter, 10);
% 
%         scatter(x,y_inter, 20, "black", "filled"); hold on
%         [ch_corrs(ch),cp] = corr(x,y_inter, 'Rows', 'complete');
%         p = polyfit(x,y_inter,1);
%         ch_coeff(ch) = p(1);
